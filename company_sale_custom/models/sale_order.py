...
class SaleOrder(models.Model):
    ...
    # Unpaid invoices from related contacts
    unpaid_related_invoice_ids = fields.Many2many(
        "account.move",
        compute="_compute_unpaid_related_invoices",
        string="Unpaid Related Contacts' Invoices",
        readonly=True,
    )
    unpaid_related_invoice_count = fields.Integer(
        compute="_compute_unpaid_related_invoices",
        string="Unpaid Related Invoices Count",
        readonly=True,
    )

    def _compute_unpaid_related_invoices(self):
        Move = self.env["account.move"]
        for order in self:
            partner = order.partner_id
            if not partner:
                order.unpaid_related_invoice_ids = Move.browse()
                order.unpaid_related_invoice_count = 0
                continue
            related_partners = partner.relation_ids.mapped("related_partner_id")
            if not related_partners:
                order.unpaid_related_invoice_ids = Move.browse()
            ...
            moves = Move.search(domain, order="invoice_date desc, id desc", limit=50)
            # Sort by payment status then date
            moves = moves.sorted(key=self._invoice_sort_key)[:10]
            order.unpaid_related_invoice_ids = moves
            # Count all (no limit) cheaply by read_group
            data = Move.read_group(domain, ["partner_id"], ["partner_id"])
            order.unpaid_related_invoice_count = sum(d.get("partner_id_count", 0) for d in data)

    def action_view_unpaid_related_invoices(self):
        self.ensure_one()
        partner = self.partner_id
        related_partners = partner.relation_ids.mapped("related_partner_id") if partner else self.env["res.partner"]
        if not related_partners:
