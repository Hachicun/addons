...
class SaleSubscription(models.Model):
    ...
    auto_generate_invoice = fields.Boolean(
        default=True, string="Auto Generate Invoice",
        help="Nếu bật, cron job sẽ tự động tạo SO+Invoice khi đến recurring_next_date"
    )
    days_until_expiry = fields.Integer(
        compute="_compute_days_until_expiry", store=False,
        string="Days Until Expiry"
    )
    expiring_soon = fields.Boolean(
        compute="_compute_expiring_soon", store=True,
        string="Expiring Soon",
        help="True nếu subscription còn ≤5 ngày nữa sẽ hết hạn"
    )
    total_periods_ordered = fields.Integer(
        compute="_compute_periods_summary", store=True,
        string="Tổng chu kỳ đã đặt"
    )
    total_periods_delivered = fields.Integer(
        compute="_compute_periods_summary", store=True,
        string="Đã giao"
    )
    remaining_periods = fields.Integer(
        compute="_compute_periods_summary", store=True,
        string="Còn lại"
    )
    next_delivery_date = fields.Date(
        compute="_compute_periods_summary", store=True,
        string="Ngày giao tiếp theo"
    )
    periods_summary_display = fields.Char(
        compute="_compute_periods_summary", store=False,
        string="Tóm tắt chu kỳ"
    )
    ...
    def cron_subscription_management(self):
        ...
        for subscription in self.search([], order="recurring_next_date asc"):
            ...
            if subscription.in_progress:
                if (
                    subscription.auto_generate_invoice
                ...
                if subscription.auto_generate_invoice:
                    subscription.generate_invoice()
    ...
    def _prepare_sale_order(self, line_ids=False, commitment_date=False):
        self.ensure_one()
        return {
            "partner_id": self.partner_id.id,
            "fiscal_position_id": self.fiscal_position_id.id,
            "date_order": commitment_date or datetime.now(),
    ...
    def _prepare_account_move(self, line_ids, invoice_date=False):
        self.ensure_one()
        values = {
            "partner_id": self.partner_id.id,
    ...
    def create_invoice(self, invoice_date=False):
        ...
        invoice_values = self._prepare_account_move(line_ids, invoice_date)
        invoice_id = (
    ...
    def create_sale_order(self, commitment_date=False):
        ...
        for line in self.sale_subscription_line_ids:
            ...
            line_ids.append(Command.create(line_values))
        values = self._prepare_sale_order(line_ids, commitment_date)
    ...
    @api.depends("date", "in_progress")
    def _compute_days_until_expiry(self):
        today = date.today()
        for record in self:
            if record.date and record.in_progress:
                delta = record.date - today
                record.days_until_expiry = max(0, delta.days)
            else:
                record.days_until_expiry = 0

    @api.depends("days_until_expiry", "in_progress", "date")
    def _compute_expiring_soon(self):
        for record in self:
            record.expiring_soon = (
                record.in_progress
                and record.date
                and record.days_until_expiry <= 5
                and record.days_until_expiry >= 0
            )

    @api.depends("sale_order_ids", "sale_order_ids.commitment_date", "sale_order_ids.state", 
                 "sale_order_ids.picking_ids", "sale_order_ids.picking_ids.state", 
                 "date_start", "template_id", "recurring_next_date")
    def _compute_periods_summary(self):
        for record in self:
            # Đếm tổng số SO đã tạo (đã đặt)
            all_orders = record.sale_order_ids
            record.total_periods_ordered = len(all_orders)
            
            # Đếm số SO đã giao (có picking done)
            delivered_orders = all_orders.filtered(
                lambda so: any(p.state == 'done' for p in so.picking_ids)
            )
            record.total_periods_delivered = len(delivered_orders)
            
            # Tính số chu kỳ còn lại (SO draft hoặc chưa giao)
            record.remaining_periods = record.total_periods_ordered - record.total_periods_delivered
            
            # Tính ngày giao hàng tiếp theo (SO draft gần nhất)
            draft_orders = all_orders.filtered(
                lambda so: so.state == 'draft' and so.commitment_date
            ).sorted('commitment_date')
            if draft_orders:
                record.next_delivery_date = draft_orders[0].commitment_date
            else:
                # Nếu không có SO draft, tính từ recurring_next_date
                if record.recurring_next_date:
                    record.next_delivery_date = record.recurring_next_date
                else:
                    record.next_delivery_date = False
            
            # Tạo text hiển thị
            info_parts = [
                f"Đã đặt: {record.total_periods_ordered}",
                f"Đã giao: {record.total_periods_delivered}",
                f"Còn lại: {record.remaining_periods}"
            ]
            if record.next_delivery_date:
                info_parts.append(f"Tiếp theo: {record.next_delivery_date.strftime('%d/%m/%Y')}")
            record.periods_summary_display = " | ".join(info_parts)
    ...
    def action_renew_subscription(self):
        """Mở wizard để gia hạn subscription"""
        self.ensure_one()
        return {
            "name": "Gia hạn Subscription",
            "view_type": "form",
            "view_mode": "form",
            "res_model": "renew.subscription.wizard",
            "type": "ir.actions.act_window",
            "target": "new",
            "context": {
    ...
    def _extend_subscription_date(self, periods):
        """Kéo dài date của subscription thêm số chu kỳ"""
        self.ensure_one()
        if not self.date or self.recurring_rule_boundary:
            # Nếu unlimited, không cần kéo dài
            return
        
        if not self.template_id:
            return
        
        # Tính date mới = date hiện tại + (periods * interval)
        type_interval = self.template_id.recurring_rule_type
        interval = int(self.template_id.recurring_interval)
        
        self.date = self.date + relativedelta(**{
            type_interval: interval * periods
        })

    def _create_renewal_orders(self, periods):
        """Tạo SO và Invoice cho từng chu kỳ gia hạn với commitment_date tương ứng"""
        self.ensure_one()
        orders_created = []
        invoices_created = []
        
        if not self.template_id:
            return orders_created, invoices_created
        
        # Lấy điểm bắt đầu: commitment_date của SO cuối cùng hoặc date của subscription
        existing_orders = self.sale_order_ids.filtered(
            lambda so: so.commitment_date
        ).sorted('commitment_date', reverse=True)
        
        if existing_orders:
            # Nếu đã có SO, tính từ commitment_date của SO cuối cùng
            start_date = existing_orders[0].commitment_date
        elif self.date:
            # Nếu có date, tính từ date
            start_date = self.date
        else:
            # Nếu unlimited, tính từ recurring_next_date hoặc date_start
        ...
        for i in range(1, periods + 1):
            # Tính commitment_date cho chu kỳ này
            commitment_date = start_date + relativedelta(**{
                type_interval: interval * i
            })
            
            # Tạo SO draft với commitment_date
            so = self.create_sale_order(commitment_date=commitment_date)
            orders_created.append(so)
            
            # Tạo Invoice draft với invoice_date = commitment_date
            invoice = self.create_invoice(invoice_date=commitment_date)
